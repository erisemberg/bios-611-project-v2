...
geno_file <- "Cr_WB02_miniMUGA-06242021.csv"
pheno_file <- "CC27xC3H_backcross_pheno.xlsx"
out_file <- "Rqtl_CC27xC3H_BC.csv"
Y_MT_out_file <- "Geno_CC27xC3H_Y_MT.csv"
...
# Convert identifier to one with padded numeric ID (so mouse IDs are consistent
# across genotype file, and between genotype and phenotype files)
### Don't need this for WB02 data - only last mouse had unpadded numeric ID, just 
### manually fixed it (and there is no underscore between sex and numeric ID)
# for (i in F2_start:F2_end){
#   colnames(geno)[i] <- paste(c(str_split(colnames(geno)[i], "_")[[1]][1:3], 
#                 sprintf("%04s", str_split(colnames(geno)[i], "_")[[1]][4])), 
#                 collapse="_")
# }
...
add_pheno <- function(pheno.name, new.col, col.pos){
  rqtl.type <- rqtl.type %>% add_column(placeholder.name = new.col, .before = col.pos)
  names(rqtl.type)[names(rqtl.type) == "placeholder.name"] <- pheno.name
  for (i in 3:nrow(rqtl.type)){
    rqtl.type[[pheno.name]][i] <- 
      ifelse(toupper(rqtl.type$mouse_ID[i]) %in% toupper(pheno.type$Geno_ID), 
             pheno.type[[pheno.name]][which(toupper(pheno.type$Geno_ID) == 
                                          toupper(rqtl.type$mouse_ID[i]))], NA)
  }
  return(rqtl.type)
}

# Select mice with the specified infection type 
# pheno.type <- pheno[which(pheno$infection == infection),]
# rqtl.type <- rqtl[which(rqtl$mouse_ID == "Chromosome" | 
#                         rqtl$mouse_ID == "Position_b38" |
#                         toupper(rqtl$mouse_ID) %in% toupper(pheno.type$Geno_ID)),]
pheno.type <- pheno
rqtl.type <- rqtl 

# Empty phenotype column (with first two rows empty, as required for R/qtl)
new.col = c(rep("",2), rep(NA, nrow(rqtl.type)-2))

# Add phenotype data to genotype data
col.pos = 2
for (k in 1:length(pheno.names)){
  pheno.name = pheno.names[k]
  rqtl.type <- add_pheno(pheno.name, new.col, col.pos)
  col.pos = col.pos+1
}
...
#--------------------------------Output data-----------------------------------#
#------------------------------------------------------------------------------#
rqtl.type[1:2,"mouse_ID"] <- "" # Remove chr/pos row labels

write.csv(rqtl.type, out_file, row.names=F)
